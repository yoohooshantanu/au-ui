		<script lang="ts">
			...
			import { createMissedDelivery, getMissedDeliveriesByDate, deleteMissedDelivery } from '$lib/api/missed_deliveries';
			...
			async function loadReaders() {
				...
				errorMessage = '';
				try {
					// Load ALL readers matching filters by paginating through results
					let allReaders: Subscriber[] = [];
					let pg = 1;
					while (true) {
						...
						allReaders = [...allReaders, ...(readersData.items || [])];
						if (pg >= (readersData.totalPages || 1)) break;
						pg++;
					}
					
					availableReaders = allReaders;
					checkedReaderIds = allReaders.map(r => r.id);
			...
			async function markBulkAttendance() {
				...
				try {
					...
					if (attendanceType === 'absent') {
						...
						const readersToProcess = availableReaders.filter(r => checkedReaderIds.includes(r.id));
						...
						// Mark readers as present (delete their missed deliveries for today if they exist)
						const existingMissed = await getMissedDeliveriesByDate(today);
						const readersToProcess = availableReaders.filter(r => checkedReaderIds.includes(r.id));
						
						for (const reader of readersToProcess) {
							const missedId = existingMissed.find(m => m.subscriber === reader.id)?.id;
							if (missedId) {
								try {
									await deleteMissedDelivery(missedId);
								} catch (e: any) {
									errorCount++;
									console.error(`Failed to mark present for ${reader.name}:`, e);
								}
							}
						}
						successCount = checkedReaderIds.length - errorCount;
		...
		<div class="fixed inset-0 bg-black/30 backdrop-blur-sm z-40 flex items-center justify-center p-4">
			<div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-full overflow-y-auto p-6">
				...
				{#if availableReaders.length > 0}
					<div class="mb-6">
						<h3 class="font-semibold text-gray-900 mb-3">Mark Attendance For {checkedReaderIds.length} readers</h3>
					...
					<div class="mb-6">
						<div class="flex justify-between items-center mb-3">
							<h3 class="font-semibold text-gray-900">Selected Readers ({checkedReaderIds.length} / {availableReaders.length})</h3>
						...
						<div class="max-h-60 overflow-y-auto border border-gray-200 rounded-lg">
							<table class="w-full text-sm">
								...
								<tbody class="divide-y divide-gray-200">
									{#each availableReaders as reader}
										<tr class="hover:bg-gray-50 cursor-pointer" on:click={() => {
											if (checkedReaderIds.includes(reader.id)) {
												checkedReaderIds = checkedReaderIds.filter(id => id !== reader.id);
											} else {
												checkedReaderIds = [...checkedReaderIds, reader.id];
											}
										}}>
				...
				<div class="flex justify-end gap-3">
					...
					{#if availableReaders.length > 0}
						...
						>
							...
							{/if}
							Mark {attendanceType === 'present' ? 'Present' : 'Absent'} ({checkedReaderIds.length})
						</button>
					{/if}
				</div>
			</div>
